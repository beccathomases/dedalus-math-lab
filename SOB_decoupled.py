...
# Velocity gradient entries (FIELDS; evaluate once and reuse)
ux_x = dx(ux).evaluate()
ux_y = dy(ux).evaluate()
uy_x = dx(uy).evaluate()
uy_y = dy(uy).evaluate()
...
while t < t_end - 1e-14:
    ...
    if it % 50 == 0:
        ...
        if comm.rank == 0:
            print(
                f"[C-LIN] it={it:5d} t={t:.4f}  CFL={CFL:.3e}  ||div u||_inf={div_inf:.3e}  "
        ...
        # relaxation contribution:  ∫ tr * ( -(1/tauR)(tr-2) )  =  -(1/tauR) ∫ tr*(tr-2)
        relax_term = -(1/tauR) * global_int_array(tr*(tr-2.0))

                # stretching production term for tr = cxx+cyy:
        # dt(tr)|stretch = 2*(ux_x*cxx + (ux_y+uy_x)*cxy + uy_y*cyy)

        # Make sure ALL fields are on the same grid (scale=1) before using ['g']
        u.change_scales(1)
        cxx.change_scales(1); cxy.change_scales(1); cyy.change_scales(1)

        ux = (u@ex).evaluate(); ux.change_scales(1)
        uy = (u@ey).evaluate(); uy.change_scales(1)

        ux_x = dx(ux).evaluate(); ux_x.change_scales(1)
        ux_y = dy(ux).evaluate(); ux_y.change_scales(1)
        uy_x = dx(uy).evaluate(); uy_x.change_scales(1)
        uy_y = dy(uy).evaluate(); uy_y.change_scales(1)

        # Now all of these ['g'] arrays are Nx-by-Ny (128x128)
        S = 2.0*(ux_x['g']*cxx['g'] +
                 (ux_y['g'] + uy_x['g'])*cxy['g'] +
                 uy_y['g']*cyy['g'])

        stretch_term = global_int_array(tr * S)


        rhs = -kappa_c * G2 + relax_term + stretch_term
        ...
        if prev_tr2 is None:
            ...
            if comm.rank == 0:
                print(
                    f"    tr-energy: d/dt(1/2∫tr^2)={ddt:.3e}   "
                    f"rhs(diff+relax+stretch)={rhs:.3e}   defect={(ddt-rhs):.3e}"
                )
            prev_tr2 = tr2
